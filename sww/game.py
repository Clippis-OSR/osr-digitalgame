...
# Emit a structured event for later 2D/journal.
    ...
    def _update_wilderness_condition(self) -> None:
        """Minimal deterministic travel-condition progression hook."""
        ts = getattr(self, "travel_state", None)
        if ts is None:
            return
        phases = ("clear", "wind", "rain", "fog")
        clock = int(getattr(ts, "wilderness_clock_turns", 0) or 0)
        # one condition phase per campaign day (6 watches) in wilderness-clock space
        phase_idx = ((int(getattr(self, "wilderness_seed", 0) or 0) // 7) + (clock // int(self.WATCHES_PER_DAY))) % len(phases)
        nxt = phases[int(phase_idx)]
        cur = str(getattr(ts, "travel_condition", "clear") or "clear")
        if cur != nxt:
            ts.travel_condition = nxt
            ts.condition_started_clock = int(clock)
        else:
            # normalize for older saves that may miss this field
            if int(getattr(ts, "condition_started_clock", 0) or 0) <= 0 and int(clock) > 0:
                ts.condition_started_clock = int(clock)
