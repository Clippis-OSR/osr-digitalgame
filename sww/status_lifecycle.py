...
def clear_status(actor: Any, key: str) -> bool:
    """Remove a status key via centralized lifecycle surface."""
    st = status_dict(actor)
    k = normalize_status_key(key)
    if not k:
        return False
...
def add_effect_flag(actor: Any, effect: str) -> bool:
    """Ensure a combat effect flag exists (e.g., fled)."""
...
def leave_combat(
    ...
    *,
    ...
    clear_status_keys: tuple[str, ...] = (),
) -> bool:
    """Mark an actor as out-of-fight while preserving existing semantics by default."""
    changed = False
    fx = list(getattr(actor, "effects", []) or [])
    for e in remove_effects:
        if e in fx:
            ...
            changed = True
    if fx != list(getattr(actor, "effects", []) or []):
        actor.effects = fx
    if add_effect_flag(actor, marker):
        changed = True
    for k in clear_status_keys:
        if clear_status(actor, k):
            changed = True
    return changed
